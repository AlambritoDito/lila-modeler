 ...
 */
export function grupos(): readonly { readonly titulo: string; readonly tokens: readonly TokenName[] }[] {
  const S = strings();
  const lista: { titulo: string; tokens: TokenName[] }[] = [];
  for (const token of TOKEN_NAMES) {
    ...
    else lista.push({ titulo, tokens: [token] });
  }
  return lista;
}
