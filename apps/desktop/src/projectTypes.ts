 ...
 * this: since ADR-027 the shapes live in `@lila-modeler/engine/project`, the one package both apps can
 * depend on, and this module only re-exports them so nothing else in `apps/desktop` had to move.
 *
 * The old copy weakened `StoredRun.result` to `unknown` to avoid depending on `@lila-modeler/engine`; the
