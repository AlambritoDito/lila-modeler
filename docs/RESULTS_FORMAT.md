...
- **`processing.{min,max,mean,total}`** — estadísticas de `endedAt − startedAt` (el tiempo que el elemento estuvo efectivamente ocupado procesando la instancia, sin contar espera de recurso ni de calendario). `mean = total / completed`. Fórmula: para el conjunto `P` de duraciones de processing completadas, `min = min(P)`, `max = max(P)`, `total = Σ P`, `mean = total / |P|`.
- **`resourceWait.{min,max,mean,sd,total}`** — estadísticas de `startedAt − enabledAt` (tiempo que la instancia esperó a que un recurso quedara disponible, dentro de horario de calendario). `sd` es la desviación estándar muestral (`n−1` en el denominador) del mismo conjunto. Si el elemento no requiere recursos (`resources` vacío en el escenario), toda instancia tiene `resourceWait = 0` — degradación de capacidad infinita (ADR-016 y sección 6).
- **`offHoursWait.{min,max,mean,sd,total}`** — mismo cálculo que `resourceWait` pero sobre el tiempo que la instancia estuvo esperando porque el calendario del recurso (o del elemento, si no requiere recurso) estaba cerrado — se acumula por separado de `resourceWait`, no se suma a él (ADR-016: "el tiempo cerrado se reporta como `offHoursWait`, separado de `resourceWait`"). Si el elemento usa calendario 24×7 (default sin calendario asignado), `offHoursWait` es siempre 0.
...
- **`costPerCase`** — `totalCost / completed`.
...
| `resourceWait` | number | segundos | `startedAt − enabledAt` menos la porción de esa espera atribuible a calendario cerrado; ver `offHoursWait`. |
| `offHoursWait` | number | segundos | Porción de `startedAt − enabledAt` en la que el calendario del recurso (o del elemento) estaba cerrado. `resourceWait + offHoursWait = startedAt − enabledAt`. |
| `cost` | number | `run.currency` | Costo atribuido a esta fila: `fixedCost` del elemento (si esta fila lo completa) más, si tiene `resourceId`, la porción de `unitCost` del recurso correspondiente a `endedAt − startedAt`. |
