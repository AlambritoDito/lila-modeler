...
|---|---|---|---|---|
| `processingTime` | distribution (§ 3) | tasks, timers | no time (0 s) | Duration of the work, in seconds. On an intermediate timer it is the delay, with no resource. |
...
| `conditions` | `elements[flow]`, `elements[task]` | Branching by an expression over case data, instead of by probability. |
...
| `conditions` (reserved) | `bpsim:ControlParameters/bpsim:Condition` † | — | — |
