...
`App.tsx` conoce los temas integrados por id (`eva-01`, `papel`, `tieso`, `akira`, `montana`,
...
`{ id, tema: { name, tokens }, origen }`: `id` es `u:<n>` (los integrados son `eva-01`, `papel`, `tieso`,
`akira`, `montana`, `lila-light` y `lila-dark`) y
