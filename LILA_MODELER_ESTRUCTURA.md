...
- **Qué se construye primero**: un paquete `@lila-modeler/engine` (motor puro + parser BPMN + formato de escenario + CLI `lila`) y una SPA (React + bpmn-js) que corre el motor en un Web Worker, empaquetada como app de escritorio con Electron. Cero backend, cero base de datos, cero Python.
...
├── packages/
│   ├── engine/                   # @lila-modeler/engine — el producto. Todo lo que no es UI vive aquí
...
│   └── mcp/                      # M4 — @lila-modeler/mcp: 5 tools sobre @lila-modeler/engine, transporte stdio
├── apps/
...
│           ├── worker.ts         # import { simulate } from '@lila-modeler/engine'; onmessage → postMessage
...
*Aceptación*: desde Claude Code con el servidor configurado, "simula examples/pedido con as-is y dime el cuello de botella" devuelve el mismo `bottlenecks[0]` que `lila run`; "qué pasa si agrego un cajero" produce un escenario con `extends` y una comparación coherente, sin ningún cambio en `@lila-modeler/engine`.
...
Regla de enganche única: cada módulo nuevo consume los contratos de `@lila-modeler/engine` (IR, escenario, resultado, event log, descriptor `lila`) y sus funciones puras; ninguno modifica el motor ni introduce un segundo modelo canónico.
...
- **ARCHITECTURE.md**: el diagrama (Web UI → REST/WebSocket → Application API → PostgreSQL, "DES Scheduler SimPy/other") describe la plataforma final; añadir el diagrama MVP de dos cajas (`apps/web` con Worker ↔ `@lila-modeler/engine`; CLI ↔ `@lila-modeler/engine`). Reescribir "Internal model ≠ BPMN XML" como "el .bpmn es canónico; la base de datos es un índice derivado". Quitar "Python API" de las interfaces del motor. Añadir la lista de elementos `lila:` y el layout `processes/<clave>/`. En Persistence, quitar SQLite/in-memory como prototipos: el prototipo es archivos + git.
