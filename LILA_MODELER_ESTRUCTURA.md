...
**Event log**: filas `{ replication, caseId, elementId, resourceId?, enabledAt, startedAt, endedAt, resourceWait, offHoursWait, cost }`; se emite por callback `opts.onEvent` para que la web agregue o muestree (30 replicaciones × 10k casos × 20 actividades = 6 M filas no caben cómodamente en memoria de navegador) y la CLI escriba en streaming. Timestamps ISO derivados de `run.start` al exportar. Es el formato plano que XES (IEEE 1849) y OCEL 2.0 / pm4py consumen tras un mapeo trivial.
